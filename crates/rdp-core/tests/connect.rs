//! The acceptance test for the wire layer as it stands: drive a session
//! through X.224 and MCS to the end of channel connection, over a real TCP
//! socket, against a mock that encodes with the same encoders the client
//! parses with (PRDRDP/12 §8.4).
//!
//! # What this covers and what it cannot
//!
//! The production functions are used unchanged:
//! `connection::negotiate_security` for phase 1 and
//! `connection::after_upgrade` for phases 2c to 4. What sits between them in
//! a real session is the TLS handshake, and the mock has no certificate to do
//! one with, so the two halves are driven over the same plain socket. That is
//! a real composition rather than a bypass: `connection::connect` calls the
//! same two functions with `transport::upgrade_tls` between them, and the
//! split exists because the handshake needs the whole stream rather than a
//! read half.
//!
//! The missing piece is a committed test key pair
//! (PRDRDP/12 §3.14 `tests/common/certs.rs`), which is what would let the
//! mock stand up a TLS listener and the whole of `connection::connect` run
//! end to end.
...
use common::mock_rdp_server::{
    McsBehaviour, MockConfig, MockRdpServer, Negotiation, IO_CHANNEL_ID, MESSAGE_CHANNEL_ID,
    USER_CHANNEL_ID,
};
...
use rdp_core::{ConnectStage, RdpError};
...
///
/// Returns the error at whichever phase the sequence reached, which for a
/// well behaved server today is the Client Info PDU.
...
///
/// Every phase the wire layer can carry today runs against a real socket, and
/// the sequence stops at the first one it cannot, naming it.
#[tokio::test]
async fn a_session_reaches_the_end_of_channel_connection() {
    let (server, events, result) =
        run_sequence(MockConfig::default(), Credentials::default()).await;

    // The stop is at the Client Info PDU and it says so. This assertion is
    // what changes when `rdp-pdu`'s `rdp/` module lands, which is the point
    // of writing it this way rather than asserting on a panic message.
    match result {
        Err(RdpError::NotImplemented { stage }) => {
            assert_eq!(stage, ConnectStage::SendClientInfo);
        }
        other => panic!("expected the sequence to stop at the Client Info PDU, got {other:?}"),
    }
    ...
    // RDP, so `Negotiating` is what the MCS phases show.
...
async fn channel_join_confirms_are_accepted_in_any_order() {
    ...
    assert!(
        matches!(result, Err(RdpError::NotImplemented { .. })),
        "the joins completed and the sequence reached the next phase"
    );
...
async fn a_server_without_a_message_channel_joins_one_fewer() {
    ...
    .await;
    assert!(matches!(result, Err(RdpError::NotImplemented { .. })));
    assert_eq!(
...
/// A server that asks for CredSSP gets the one honest answer available: the
/// TLS upgrade does not hand back the certificate CredSSP binds to, so the
/// gap is named rather than worked around. Under `NlaPolicy::Required` the
/// connection fails; under `AllowFallback` it continues over TLS.
///
/// This assertion changes when `vnc_transport::tls::upgrade_with_identity`
/// lands, which is what makes it worth writing.
#[tokio::test]
async fn a_server_requiring_nla_reports_the_named_gap() {
    ...
    match result {
        Err(RdpError::NotImplemented { stage }) => {
            // Reported against the phase with the missing value, which is the
            // TLS upgrade rather than CredSSP.
